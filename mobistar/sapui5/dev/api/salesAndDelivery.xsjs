...
function GetReceveing() {
	...
	try {
		...
		for (var i = 0; i < firstQryRslt.length; i++) {
			...
			var arr1 = [];
			query4 =
				'select  count(IMEI1)  from "MDB_DEV"."SALES_INVOICE_EQUIP" AS E  inner join "MDB_DEV"."MATERIALMASTER" as M on M.MATERIAL_CODE=E.MATERIAL_CODE where E.INVOICE_NO =? AND E.SAPUSER_ID=?  '; //AND (PROCESS_STATUS=? )
		...
		//	pstmt4.setString(3, '1');
			...
			query5 =
				'select count(INVOICE_NO) as INVOICECOUNT  from "MDB_DEV"."SALES_INVOICE_EQUIP" where SAPUSER_ID =? AND INVOICE_NO=? and MATERIAL_CODE is not null'; // AND  (PROCESS_STATUS =? ) 
			...
			for (var m = 0; m < oFinalResult[i].INVOICE_ITEMS.length; m++) {
				for (var k = 0; k < oFinalResult[i].INVOICE_ITEMS[m].INVOICE_IMEI.length; k++) {
					if (oFinalResult[i].INVOICE_ITEMS[m].INVOICE_IMEI[k].IsChecked === '1') {
						...
						var queryinsert =
							' insert into "MDB_DEV"."EQUIPMENT_MASTERTRANS" ("INVOICE_NO", "TRANSACTION_DATE" ,"SERIAL_NO" , "MATERIAL_CODE" , "IMEI1" , "SAPUSER_ID" , "ACTIVE") values (? , ? , ? , ? , ? , ? , ? ) ';
...
function getModelByImei() {
	...
	try {
		var queryGetEqDetails =
			'select e.MATERIAL_CODE,e.SERIAL_NUMBER ,p.MODEL_CODE , p.MODEL_DESCRIPTION from "MDB_DEV"."EQUIPMENT_MASTER" as e inner join "MDB_DEV"."MATERIALMASTER" as p on p.MATERIAL_CODE=e.MATERIAL_CODE inner join "MDB_DEV"."SALES_INVOICE_EQUIP" as eq on eq.IMEI1 = e.IMEI1 where e.IMEI1 = ? and eq.SAPUSER_ID=?  ';
		...
		while (rsGetEqDetails.next()) {
			...
			query = 'select PARENT_CUST_NAME,PARENT_CUST_CODE FROM "MDB_DEV"."MST_CUSTOMER" WHERE SAPUSER_ID=? ';
		...
		if (dataLine.length > 0) {
			for (var i = 0; i < dataLine.length; i++) {
				...
				for (var j = 0; j < dataLine[i].IMEIS_NO.length; j++) {
					...
					var querySelectequipment = 'select IMEI1,MATERIAL_CODE FROM  "MDB_DEV"."EQUIPMENT_MASTER" WHERE IMEI1=?';
					...
					if (rsSelectequipment.next()) {
						var query =
							'select CUST_TYPE,SAPUSER_ID,CUST_NAME FROM "MDB_DEV"."MST_CUSTOMER" WHERE SAPUSER_ID=? and PARENT_CUST_CODE=? ';//and STATUS=?
						...
						if (rpstm0.next()) {
							...
							if (usertype === rpstm0.getString(1)) {
								var queryinsertequipment =
									'insert into "MDB_DEV"."EQUIPMENT_MASTERTRANS" (SERIAL_NO ,MATERIAL_CODE ,IMEI1 ,ACTIVE ,SAPUSER_ID ,STATUS , TRANSACTION_DATE ,INVOICE_NO) values(?,?,?,?,?,?,?,?)';
								var statusid = '3';
