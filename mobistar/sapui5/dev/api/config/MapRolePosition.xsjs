...
function getMapRolePosition() {
	...
	try {
		var CallPro = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::MAP_ROLE_POSITION"(?,?,?,?,?,?,?,?,?);';
...
function getRolePositions(){
	...
	try {
        var qryRole = 'Select Role_id,role_name from "MDB_DEV"."MST_ROLE" where SOFT_DEL = ?';    	
    	...
    	while (rsRole.next()) {
    		...
    		var qryRolePosi = 'select RolePosi.role_pos_id,RolePosi.position_id,Position.position_name from "MDB_DEV"."MAP_ROLE_POSITION" as RolePosi inner join "MDB_DEV"."MST_POSITION" as Position on RolePosi.position_id = Position.position_id where ROLE_ID = ? and RolePosi.SOFT_DEL = ? and Position.SOFT_DEL = ?';
...
function addMapRolePosition() {
	...
	try {
		...
		pstmtCallPro.setString(5, createby);
		pstmtCallPro.setString(6, dateFunction());
		pstmtCallPro.setString(7, '');
		pstmtCallPro.setString(8, dateFunction());
...
function updateRolePosition() {
	...
	try {
		var UpdCallPro = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::MAP_ROLE_POSITION"(?,?,?,?,?,?,?,?,?);';
...
function deleteRolePosition() {
	...
	try {
		var DelCallPro = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::MAP_ROLE_POSITION"(?,?,?,?,?,?,?,?,?);';
