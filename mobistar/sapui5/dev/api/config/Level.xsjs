 ...
 */
 ...
 function getLevel() {
 	...
 	var rsLevel = '';
 	try {
 		queryLevel = ' SELECT * FROM "MDB_DEV"."MST_LEVEL" WHERE LEVEL_ID=? ';
 ...
 function getLevels() {
 	...
 	try {
 		var CallPro = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::MST_LEVEL"(?,?,?,?,?,?,?,?)';
...
function fetchLevel(inputData, connection) {
	var queryselectLevel = 'select * from "MDB_DEV"."MST_LEVEL" where SOFT_DEL is not null ';
	var whereClause = "";
	if (inputData.level !== '') {
...
var modifyby = $.request.parameters.get('modifyby');
 	try {
 		var UpdCallPro = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::MST_LEVEL"(?,?,?,?,?,?,?,?);';
 		...
 		pstmtUpdCallPro.setInteger(3, 0);//parseInt(levelNameEditStatus,10)
