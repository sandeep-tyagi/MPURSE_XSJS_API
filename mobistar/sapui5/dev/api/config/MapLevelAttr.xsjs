...
function checkLevelRecord(rSubMenu, rAttr, records, levelId) {
	...
	var queryCheckLvlAtt = 'select * from "MDB_DEV"."MAP_LEV_ATTR" where SUB_MENU_ID = ? and ATTRIBUTE_ID = ? and SOFT_DEL = ? and LEVEL_ID=?';
...
function getSubMenuLevelAttr() {
	...
	try {
		...
		while (rMenu.next()) {
			...
			var record = {};
			var SubMenu = [];
			var querySubMenu = 'select distinct sub_menu_name,submenu_id,menu_id from "MDB_DEV"."MST_SUB_MENU" where menu_id = ? and SOFT_DEL = ?';
...
function getMapLevAttrs() {
	...
	try {
		var CallLvlAttr = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::MAP_LEV_ATTR"(?,?,?,?,?,?,?,?,?,?)';
...
function deleteMapLevelAttr(dataLine, status) {
	...
	if (dataLine.length > 0) {
		...
		var qryDeleteLevelAtt =
			'Delete from "MDB_DEV"."MAP_LEV_ATTR" where LEVEL_ID = ? and SUB_MENU_ID in (select SUBMENU_ID from "MDB_DEV"."MST_SUB_MENU" where MENU_ID = ?)';
