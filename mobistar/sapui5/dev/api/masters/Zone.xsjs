...
function deleteZone() {
	...
	var record = {};
	try {
			var qryDeleteZone = 'update "MDB_DEV"."MST_ZONE" set SOFT_DEL=? where ZONE_CODE=?';
...
function updateZone() {
	...
	var record = {};
	try {
			var qryDeleteRegion = 'update  "MDB_DEV"."MST_ZONE"  set ZONE_DESC=? , SOFT_DEL=?,MODIFIED_BY=?,MODIFIED_DATE=? where ZONE_CODE=? and AREA_CODE=?';
...
function getZone() {
	...
	try {
		var query = 'Select ZONE_CODE ,ZONE_DESC,AREA_CODE from "MDB_DEV"."MST_ZONE" where ZONE_CODE = ?';
	...
	try {
		if (dataLine.length > 0) {
			for (var i = 0; i < dataLine.length; i++) {
				...
				var queryZone = 'select ZONE_CODE  from "MDB_DEV"."MST_ZONE" where ZONE_CODE = ?';
				...
				if (rZone.next()) {
					...
					var queryaddZone =
						'insert into  "MDB_DEV"."MST_ZONE"("ZONE_CODE","ZONE_DESC","AREA_CODE","CREATE_BY") values(?,?,?,?)';
...
function getZoneDetails(zoneCode, output) {
	...
	try {
		var connection = $.db.getConnection();
		var CallPro = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::Zone_CRUD"(?,?,?,?);';
		...
		while (rsCallPro.next()) {
			...
			var CallProBRANCH = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::Zone_CRUD"(?,?,?,?)';
