...
function deleteState() {
	...
	var record = {};
	try {
		var qryDeleteState = 'update "MDB_DEV"."MST_STATE" set SOFT_DEL=? where STATE_CODE=?';
...
function updateState() {
   ...
   var modifiedBy =  $.request.parameters.get('modifiedBy');
	try {
		record = {};
		var CallPro = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::State_CRUD"(?,?,?,?,?,?,?,?,?,?)';
...
function getRegionStates() {
	...
	try {
		...
		queryState = 'SELECT * FROM  "MDB_DEV"."MST_STATE" WHERE REGION_CODE=?';
		pstmtState = connection.prepareStatement(queryState);
...
function fetchState(inputData,connection){
    var queryselectState = 'select * from "MDB_DEV"."MST_STATE" where SOFT_DEL is not null ';
    var whereClause = "";
...
function getStateDetails(stateCode, output) {
	...
	try {
		...
		var CallProDistrict = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::State_CRUD"(?,?,?,?,?,?,?,?,?,?)';
		...
		while (rsCallProDistrict.next()) {
			...
			var CallProAREA = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::State_CRUD"(?,?,?,?,?,?,?,?,?,?)';
			...
			while (rsCallProAREA.next()) {
				...
				var CallProZONE = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::State_CRUD"(?,?,?,?,?,?,?,?,?,?)';
				...
				while (rsCallProZONE.next()) {
					...
					var CallProBRANCH = 'call "MDB_DEV"."com.mobistar.sapui5.dev.procedure::State_CRUD"(?,?,?,?,?,?,?,?,?,?)';
