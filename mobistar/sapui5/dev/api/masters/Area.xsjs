 	...
 	try {
 		...
 		if (areaStatus === "Deleted") {
 			qryAreas = 'SELECT ma.AREA_CODE, '
 			 ...
 			 +  ' ms.STATE_NAME, '
 			 +  ' mr.REGION_NAME, '
 			 +  ' ma.AREA_NAME ,tr.REGIONAL_CODE '
 			 + ' FROM   "MDB_DEV"."MST_AREA" AS ma '
 			 + ' inner join "MDB_DEV"."MST_DISTRICT" AS md '
 			 +  '      ON ma.DISTRICT_CODE = md.DISTRICT_CODE '
 			 + ' inner join "MDB_DEV"."TRN_REGIONAL" AS tr '
 		 +     ' ON md.STATE_CODE = tr.STATE_CODE'
 		 ...
 		 + ' inner join "MDB_DEV"."MST_REGION" as mr on ms.REGIONCODE = mr.REGION_CODE'
 		  + ' WHERE ma.SOFT_DEL = \'1\' ';
 			/*'SELECT ma.AREA_CODE,ma.AREA_DESC,ma.DISTRICT_CODE,ma.SOFT_DEL,ma.CREATE_BY,ma.CREATE_DATE, '
                            + 'ma.MODIFIED_BY,ma.MODIFIED_DATE,ma.MODIFIED_DATE ,ma.AREA_NAME from "MDB_DEV"."MST_AREA" as ma where SOFT_DEL =\'1\'';*/
 		 ...
 		 + ' inner join "MDB_DEV"."MST_REGION" as mr on ms.REGIONCODE = mr.REGION_CODE'
 		  + ' WHERE ma.SOFT_DEL = \'0\' ';
 			/*'SELECT  ma.AREA_CODE,ma.AREA_DESC,ma.DISTRICT_CODE,ma.SOFT_DEL,ma.CREATE_BY,ma.CREATE_DATE, ' +
 				'ma.MODIFIED_BY,ma.MODIFIED_DATE,ma.MODIFIED_DATE ,ma.AREA_NAME  from "MDB_DEV"."MST_AREA" as ma where SOFT_DEL=\'0\'';*/
 		} else {
 			qryAreas = ' select  ma.AREA_CODE,ma.AREA_DESC,ma.DISTRICT_CODE,ma.SOFT_DEL,ma.CREATE_BY,ma.CREATE_DATE,md.DISTRICT_NAME,ms.STATE_NAME,mr.REGION_NAME,ma.AREA_NAME ,md.REGIONAL_CODE from "MDB_DEV"."MST_AREA" AS ma '
 		 ...
 		 //+ ' inner join "MDB_DEV"."MST_REGION" as mr on ms.REGIONCODE = mr.REGION_CODE';
 			...
 			/*' SELECT ma.AREA_CODE,ma.AREA_DESC,ma.DISTRICT_CODE,ma.SOFT_DEL,ma.CREATE_BY,ma.CREATE_DATE, ' +
 				' md.DISTRICT_NAME,ms.STATE_NAME,mr.REGION_NAME,ma.AREA_NAME ' + ' from "MDB_DEV"."MST_AREA" as ma ' +
 				' inner join "MDB_DEV"."MST_DISTRICT" as md on ma.DISTRICT_CODE = md.DISTRICT_CODE ' +
 		...
 		while (rsAreas.next()) {
 			...
 			record.REGIONAL_CODE = rsAreas.getString(11);
 ...
 function updateArea() {
 	...
 	try {
 		if (!validateAreaName(areaName, connection, record)) {
 			var qryDeleteArea =
 				'update  "MDB_DEV"."MST_AREA" set 	AREA_DESC=? ,SOFT_DEL=? ,MODIFIED_BY=?,MODIFIED_DATE=?,AREA_NAME=? where AREA_CODE=? and DISTRICT_CODE=?';
