...
function getEmpAuthDetail(record, output) {
	var connection = $.db.getConnection();
	var CallPro = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.testintegration.procedure::UserAuthDetail"(?,?,?)';
	var pstmtCallPro = connection.prepareCall(CallPro);
...
function getUserAndPassword() {
	...
	try {
		var query = 'select * from "MDB_TEST_INTEGRATION"."USER_REGISTRATION" where USER_CODE=? and PASSWORD=?';
...
function getUserDetails(record, userId) {
	var connection = $.db.getConnection();
	var queryUser = 'select PASSWORD,CHANGE_PASSWORD from "MDB_TEST_INTEGRATION"."USER_REGISTRATION" where USER_CODE = ? ';
...
function changePassword() {
	...
	try {
		...
		var qryChangePass = 'update "MDB_TEST_INTEGRATION"."USER_REGISTRATION" set CHANGE_PASSWORD=? , PASSWORD = ? where USER_CODE=?';
...
function getEmployeeRole(dicLine) {
	var conn = $.db.getConnection();
	var query = 'select EM.EMPLOYEE_CODE,RO.ROLE_NAME from "MDB_TEST_INTEGRATION"."MST_EMPLOYEE" as EM ' +
		' inner join "MDB_TEST_INTEGRATION"."MAP_ROLE_POSITION" as RP on EM.ROLE_POSITION_ID = RP.ROLE_POS_ID' +
		' inner join "MDB_TEST_INTEGRATION"."MST_ROLE" as RO on RP.ROLE_ID = RO.ROLE_ID' + ' where EM.EMPLOYEE_CODE = ?';
	var pstmt = conn.prepareStatement(query);
	...
	try {
		if (dataLine.length > 0) {
			for (var i = 0; i < dataLine.length; i++) {
				...
				record = {};
				var queryCheckUser = 'select * from "MDB_TEST_INTEGRATION"."USER_REGISTRATION" where USER_CODE = ?';
