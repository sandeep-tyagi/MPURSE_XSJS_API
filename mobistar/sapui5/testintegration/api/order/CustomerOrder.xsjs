...
function getPORange(record) {
	...
	var queryRange = 'select ID,name from "MDB_TEST_INTEGRATION"."PO_RANGE"';
...
function generatePOUpdate(record) {
	...
	id = id + 1;
	var query2 = 'UPDATE "MDB_TEST_INTEGRATION"."PO_RANGE" SET ID = ?,value = ? WHERE name = ?';
...
function updateSoPo(dicLine){
    ...
    var conn = $.db.getConnection();
	var query2 = 'UPDATE "MDB_TEST_INTEGRATION"."SALESORDER_LINE" SET PARENT_PO_NUMBER = ?,PARENT_PO_DATE = ? WHERE SO_NUMBER = ? and MATERIAL_CODE = ?';
	var pstmt2 = conn.prepareStatement(query2);
...
function getSOStatus(dicLine,dicHeader){
    var conn = $.db.getConnection();
    var queryStatus = 'select SO_STATUS from "MDB_TEST_INTEGRATION"."SALESORDER_HEADER" where PARENT_CODE = ? and SO_NUMBER = ?';
...
function updateSoStatus(dicLine,dicHeader){
    ...
    if(dicLine.SoStatus === '0'){
        ...
        var conn = $.db.getConnection();
	var query2 = 'UPDATE "MDB_TEST_INTEGRATION"."SALESORDER_HEADER" SET SO_STATUS = ? WHERE PARENT_CODE = ? and SO_NUMBER = ?';
	var pstmt2 = conn.prepareStatement(query2);
	...
	try {
		...
		if (dataHeader.length > 0 && dataLine.length > 0) {
			for (var i = 0; i < dataHeader.length; i++) {
				...
				var queryHeader =
					'insert into  "MDB_TEST_INTEGRATION"."PO_HEADER"(PARENT_CODE,CUST_CODE,PO_NUMBER,PO_STATUS,CREATED_BY)values(?,?,?,?,?)';
			...
			for (var j = 0; j < dataLine.length; j++) {
				...
				var queryLine =
					'insert into  "MDB_TEST_INTEGRATION"."PO_LINE"(MATERIAL_CODE,ORDER_QTY,UNIT_PRICE,REQUIRED_DATE,DISCOUNT,SGST,IGST,CGST,TOTAL_AMT,CREATED_BY,PO_NUMBER,PO_LINE_NO)values(?,?,?,?,?,?,?,?,?,?,?,?)';
...
function getSoAndPOAccordingCustDSTB(CustCode,output){
    ...
    var record;
    var queryHeader =
			'select c.cust_name,SH.CUST_CODE,SH.SO_NUMBER,SH.CHILD_PO_NUMBER,SH.CHILD_PO_DATE from "MDB_TEST_INTEGRATION"."SALESORDER_HEADER" as SH inner join "CELEKONDMS"."CUSTOMER_MASTER" as c on SH.CUST_CODE = c.CUST_CODE where SH.PARENT_CODE = ? and SO_STATUS=?';
		...
		while (rsHeader.next()) {
			...
			var queryLineItems =
				'select Material_code,order_qty,unit_price,required_date,total_amt,SO_NUMBER from "MDB_TEST_INTEGRATION"."SALESORDER_LINE" where SO_NUMBER = ?';
...
function getSoAndPOAccordingCustSTK(CustCode,output){
    ...
    var record;
    var queryHeader =
			'select c.cust_name,PH.CUST_CODE,PH.PO_NUMBER,PH.PO_DATE from "MDB_TEST_INTEGRATION"."PO_HEADER" as PH inner join "CELEKONDMS"."CUSTOMER_MASTER" as c on PH.CUST_CODE = c.CUST_CODE where PH.PARENT_CODE = ? and PO_STATUS=?';
		var pstmtHeader = conn.prepareStatement(queryHeader);
		pstmtHeader.setString(1, CustCode);
		pstmtHeader.setInteger(2, 0);
		var rsHeader = pstmtHeader.executeQuery();
		...
		while (rsHeader.next()) {
			...
			var queryLineItems =
				'select Material_code,order_qty,unit_price,required_date,total_amt,PO_NUMBER from "MDB_TEST_INTEGRATION"."PO_LINE" where PO_NUMBER = ?';
...
function getSOLinesItemsDetail() {
	...
	try {
		var CallPro = 'call "MDB_TEST_INTEGRATION"."MobiProcedure::GetSOLineItems"(?)';
...
function getSOMasterHeaderDetail() {
	...
	var CustCode = $.request.parameters.get('CustCode');
	try {
		var CallPro = 'call "MDB_TEST_INTEGRATION"."MobiProcedure::GetSOMstHeader"(?)';
		var pstmtCallPro = conn.prepareCall(CallPro);
...
function salesOrderRange(record) {
	...
	var conn = $.db.getConnection();
	var queryRange = 'select ID,name from "MDB_TEST_INTEGRATION"."SO_RANGE"';
	var pstmtRange = conn.prepareStatement(queryRange);
	var rrange = pstmtRange.executeQuery();
	conn.commit();
	while (rrange.next()) {
...
function salesOrderUpdate(record) {
	...
	var id = parseInt(record.ID, 10);
	id = id + 1;
	var query2 = 'UPDATE "MDB_TEST_INTEGRATION"."SO_RANGE" SET ID = ?,value = ? WHERE name = ?';
	var pstmt2 = conn.prepareStatement(query2);
	pstmt2.setInteger(1, id);
	...
	try {
		...
		if (dataHeader.length > 0 && dataLine.length > 0) {
			for (var i = 0; i < dataHeader.length; i++) {
				var dicHeader = dataHeader[i];
				var queryHeader =
					'insert into  "MDB_TEST_INTEGRATION"."SALESORDER_HEADER"(PARENT_CODE,CUST_CODE,SHIP_TO,SO_NUMBER,SO_STATUS,CHILD_PO_NUMBER,CHILD_PO_DATE,CREATED_BY)values(?,?,?,?,?,?,?,?)';
				var pstmtHeader = conn1.prepareStatement(queryHeader);
				pstmtHeader.setString(1, dicHeader.ParentCode);
			...
			for (var j = 0; j < dataLine.length; j++) {
				...
				var dicLine = dataLine[j];
				var queryLine =
					'insert into  "MDB_TEST_INTEGRATION"."SALESORDER_LINE"(MATERIAL_CODE,ORDER_QTY,UNIT_PRICE,REQUIRED_DATE,DISCOUNT,SGST,IGST,CGST,TOTAL_AMT,CREATED_BY,SO_NUMBER,SO_LINE_NO)values(?,?,?,?,?,?,?,?,?,?,?,?)';
				var pstmtLine = conn1.prepareStatement(queryLine);
...
function getMaterialAll() {
	...
	var conn1 = $.db.getConnection();
	try {
		var CallPro = 'call "MDB_TEST_INTEGRATION"."MobiProcedure::GetMaterial"';
...
function get1MaterialPrice() {
	...
	var conn1 = $.db.getConnection();
	try {
		var CallPro = 'call "MDB_TEST_INTEGRATION"."MobiProcedure::Get1MatPrice"(?,?)';
		var pstmtCallPro = conn1.prepareCall(CallPro);
		pstmtCallPro.setString(1, CustCode);
...
function getChild() {
	...
	var conn = $.db.getConnection();
	try {
		var CallChildCust = 'call "MDB_TEST_INTEGRATION"."MobiProcedure::GetChildCust"(?)';
