...
function addCustRemarks(userCode, remarks, formId, connection, record) {
	...
	var qryaddCustRemarks = 'INSERT INTO "MDB_TEST_INTEGRATION"."REMARKS_HISTORY" (DBR_FORM_ID,APPROVAL_ID,REMARKS) VALUES (?,?,?)';
...
/*function addNextLevelCustApproval(formIdObj,connection,record) {
    	...
    	var qryaddNextLevel = 'INSERT INTO "MDB_TEST_INTEGRATION"."CUSTOMER_APPROVAL"(DBR_FORM_ID,APPROVAL_TYPE,APPROVAL_NAME,STATUS,APPROVAL_ID,APPROVAL_LEVEL) VALUES (?,?,?,?,?,?)';
...
function getParentEmp(userCode, connection, record) {
	...
	try {
		...
		querygetParentEmp =
			'select * from "MDB_TEST_INTEGRATION"."MST_EMPLOYEE" as m inner join "MDB_TEST_INTEGRATION"."MAP_ROLE_POSITION" as mp ' +
			' on m.ROLE_POSITION_ID = mp.ROLE_POS_ID ' +
			' inner join "MDB_TEST_INTEGRATION"."POSITION_HIERARCHY" as h on h.POSITION_ID = mp.POSITION_ID ' +
			' inner join "MDB_TEST_INTEGRATION"."MST_POSITION" as p on p.POSITION_ID = h.PARENT_POSITION_ID ' +
		...
		switch (record.ParentPosition) {
			case "DISTRICT":
				query = 'Select DISTRICT_CODE from "MDB_TEST_INTEGRATION"."MST_AREA" where AREA_CODE=?';
				break;
			case "STATE":
				query = ' Select STATE_CODE from "MDB_TEST_INTEGRATION"."MST_DISTRICT" where DISTRICT_CODE = ? ';
				break;
			case "REGION":
				query = ' Select REGION_CODE from "MDB_TEST_INTEGRATION"."MST_STATE" where STATE_CODE = ? ';
				break;
			case "COUNTRY":
				query = ' Select COUNTRY_CODE from "MDB_TEST_INTEGRATION"."MST_REGION" where REGION_CODE = ? ';
				break;
		...
		if (record.ParentPositionValue === "") {
			var qryParentRolePositionHie = 'select * from "MDB_TEST_INTEGRATION"."MST_EMPLOYEE" where POSITION_VALUE_ID=? AND ROLE_POSITION_ID=( ' +
				'SELECT PARENT_ID FROM "MDB_TEST_INTEGRATION"."MAP_ROLE_POSITION_HIERARCHY" ' +
				' WHERE ROLE_POSITION_ID = (SELECT ROLE_POSITION_ID FROM "MDB_TEST_INTEGRATION"."MST_EMPLOYEE" ' + ' WHERE EMPLOYEE_CODE= ?))';
			...
			var qryParentEmpInfo = 'select * from "MDB_TEST_INTEGRATION"."MST_EMPLOYEE" where POSITION_VALUE_ID=?';
...
function addNextLevelCustApproval(formIdObj, connection, record, userCode) {
	...
	var qryaddNextLevel =
		'INSERT INTO "MDB_TEST_INTEGRATION"."CUSTOMER_APPROVAL"(DBR_FORM_ID,APPROVAL_TYPE,APPROVAL_NAME,STATUS,APPROVAL_ID,APPROVAL_LEVEL) VALUES (?,?,?,?,?,?)';
	var pstmtaddNextLevel = connection.prepareStatement(qryaddNextLevel);
	pstmtaddNextLevel.setString(1, formIdObj.DBR_FORM_ID);
	pstmtaddNextLevel.setString(2, formIdObj.APPROVAL_TYPE);
...
function getPendDBRRequest() {
	...
	try {
		...
		querygetPendingDBR =
		// 			' Select * from "MDB_TEST_INTEGRATION"."CUSTOMER_APPROVAL" as CAPP inner join "MDB_TEST_INTEGRATION"."DBR_PROFILE" as DBRP on CAPP.DBR_FORM_ID=DBRP.DBR_FORM_ID ' +
		//           ' inner join "MDB_TEST_INTEGRATION"."DIRECTOR_PROFILE" as DIRP on DBRP.DBR_FORM_ID=DIRP.DBR_FORM_ID where CAPP.STATUS=? and CAPP.APPROVAL_ID=? ';
		/* 	' Select * from "MDB_TEST_INTEGRATION"."CUSTOMER_APPROVAL" as CAPP inner join "MDB_TEST_INTEGRATION"."DBR_PROFILE" as DBRP on CAPP.DBR_FORM_ID=DBRP.DBR_FORM_ID ' +
           ' inner join "MDB_TEST_INTEGRATION"."DIRECTOR_PROFILE" as DIRP on DBRP.DBR_FORM_ID=DIRP.DBR_FORM_ID ' +
           ' inner join "MDB_TEST_INTEGRATION"."DBR_BUSI_PROFILE" as DBRB on DIRP.DBR_FORM_ID=DBRB.DBR_FORM_ID where CAPP.STATUS=? and CAPP.APPROVAL_ID=? ';*/
		' Select * from "MDB_TEST_INTEGRATION"."CUSTOMER_APPROVAL" as CAPP inner join "MDB_TEST_INTEGRATION"."DBR_PROFILE" as DBRP on CAPP.DBR_FORM_ID=DBRP.DBR_FORM_ID ' +
...
function getEmployeeEmailId(apprecord) {
	var connection = $.db.getConnection();
	var queryEmpEmail = 'select EMAIL from "MDB_TEST_INTEGRATION"."MST_EMPLOYEE" where EMPLOYEE_CODE = ?';
...
function getDBRRegistration(formIdObj, connection, userCode) {
	...
	var queryDSTBRegs = 'select email_id,firm_name  from "MDB_TEST_INTEGRATION"."DBR_PROFILE" where  dbr_form_id = ?';
...
function dbrFormApprovalProcess(formIdObj, userCode, remarks, connection, Output) {
	try {
		...
		qryUpdDBRForm = 'UPDATE "MDB_TEST_INTEGRATION"."CUSTOMER_APPROVAL" SET STATUS=?,APPROVAL_TYPE=? WHERE DBR_FORM_ID=? and APPROVAL_ID=?';
...
function addStateHeadLoginMeeting(StateformIdObj, employeeListObj, connection, record) {
	...
	try {
		var qryAddStateHeadMeeting =
			'insert into "MDB_TEST_INTEGRATION"."STATE_HEAD_MEETING_INFO" (DBR_FORM_ID,APPROVAL_ID,EMPLOYEE_NAME,DATE) values (?,?,?,?)';
...
function addStateHeadLoginApproval() {
	...
	try {
		for (var i = 0; i < dbrStateIdArray.length; i++) {
			...
			var record = {};
			var qryGetStateHead = 'select * from "MDB_TEST_INTEGRATION"."STATE_HEAD_APPROVAL" where DBR_FORM_ID=?';
...
function updDbrFormReject(userCode, formIdObj, connection, record) {
	...
	var qryDbrRej = 'UPDATE "MDB_TEST_INTEGRATION"."DBR_PROFILE" SET STATUS=? WHERE DBR_FORM_ID=?';
...
function delDbrStateHeadMeeting(formId, connection, record) {
	...
	var qryDelSHM = 'DELETE FROM "MDB_TEST_INTEGRATION"."STATE_HEAD_MEETING_INFO" WHERE DBR_FORM_ID=?';
...
function delDbrStateHeadApproval(formId, connection, record) {
	...
	var qryDelDbrSHA = 'DELETE FROM "MDB_TEST_INTEGRATION"."STATE_HEAD_APPROVAL" WHERE DBR_FORM_ID=?';
...
function dbrFormReject() {
	...
	try {
		...
		for (var i = 0; i < dbrFormIdArray.length; i++) {
			...
			qryRejDBRForm = 'UPDATE "MDB_TEST_INTEGRATION"."CUSTOMER_APPROVAL" SET STATUS=? WHERE DBR_FORM_ID=?';
...
function getDBRForm() {
	...
	try {
		...
		querygetPendingDBR =
			' Select * from "MDB_TEST_INTEGRATION"."CUSTOMER_APPROVAL" as CAPP inner join "MDB_TEST_INTEGRATION"."DBR_PROFILE" as DBRP on CAPP.DBR_FORM_ID=DBRP.DBR_FORM_ID ' +
			' inner join "MDB_TEST_INTEGRATION"."DIRECTOR_PROFILE" as DIRP on DBRP.DBR_FORM_ID=DIRP.DBR_FORM_ID ' +
			' inner join "MDB_TEST_INTEGRATION"."DBR_BUSI_PROFILE" as DBRB on DIRP.DBR_FORM_ID=DBRB.DBR_FORM_ID where CAPP.STATUS=? and DBRP.DBR_FORM_ID=? ';
		pstmtgetPendingDBR = connection.prepareStatement(querygetPendingDBR);
...
function updateDirectorSapId(sapId, userId, record) {
	var connection = $.db.getConnection();
	var qryUpdDirc = 'update "MDB_TEST_INTEGRATION"."DIRECTOR_PROFILE" set SAPUSER_ID=? where DBR_FORM_ID=?';
...
function updateBussProfileSapId(sapId, userId, record) {
	var connection = $.db.getConnection();
	var qryUpdBussProfile = 'update "MDB_TEST_INTEGRATION"."DBR_BUSI_PROFILE" set SAPUSER_ID=? where DBR_FORM_ID=?';
...
function updateCustLegalSapId(sapId, userId, record) {
	var connection = $.db.getConnection();
	var qryUpdCustLegal = 'update "MDB_TEST_INTEGRATION"."CUSTOMER_LEGAL_INFO" set SAPUSER_ID=? where DBR_FORM_ID=?';
...
function updateFosSapId(sapId, userId, record) {
	var connection = $.db.getConnection();
	var qryUpdFos = 'update "MDB_TEST_INTEGRATION"."DBR_FOS_DETAILS" set SAPUSER_ID=? where DBR_FORM_ID=?';
...
function getDRBDetails(userId, records) {
	var connection = $.db.getConnection();
	var qrydbr = 'Select * from "MDB_TEST_INTEGRATION"."DBR_PROFILE" where DBR_FORM_ID=?';
...
function updateCustomerData(sapId, userId, record) {
	...
	var qryGetCust = 'select * from "MDB_TEST_INTEGRATION"."MST_CUSTOMER" where SAPUSER_ID=? and DBR_FORM_ID=?';
	...
	if (rgetGetCust.next()) {
		...
		var queryCust = 'insert into  "MDB_TEST_INTEGRATION"."MST_CUSTOMER" ' +
