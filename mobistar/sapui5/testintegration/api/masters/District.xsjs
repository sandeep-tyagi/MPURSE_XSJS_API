...
function getDistrict() {
	...
	try {
		var CallDistrict = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.dev.procedure::District_CRUD"(?,?,?,?);';
...
function getDistDetails(distCode, output){
	...
	try {
		var connection = $.db.getConnection();
		var CallDistDetails = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.dev.procedure::District_CRUD"(?,?,?,?);';
		...
		var CallProAREA = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.dev.procedure::District_CRUD"(?,?,?,?)';
		...
		while (rsCallProAREA.next()) {
			...
			var CallProZONE = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.dev.procedure::District_CRUD"(?,?,?,?)';
			...
			while (rsCallProZONE.next()) {
				...
				var CallProBRANCH = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.dev.procedure::District_CRUD"(?,?,?,?)';
...
function getDistricts() {
	...
	try {
		var QuerygetDistricts =
			...
			+ ' ms.STATE_CODE,ms.STATE_NAME,ms.REGION_CODE,mr.REGION_NAME,ms.COUNTRY_CODE  from "MDB_TEST_INTEGRATION"."MST_DISTRICT" as md  '
			+ ' inner join "MDB_TEST_INTEGRATION"."MST_STATE" as ms on md.STATE_CODE = ms.STATE_CODE '
			+ ' inner join "MDB_TEST_INTEGRATION"."MST_REGION" as mr on ms.REGION_CODE = mr.REGION_CODE '
			+ ' inner join  "MDB_TEST_INTEGRATION"."MST_COUNTRY" as cm on ms.COUNTRY_CODE = cm.COUNTRY_CODE ';
...
function getDistrictByStateCountry() {
	...
	try {
	    if(StateCode === '' || StateCode === undefined){
			...
			'select md.DISTRICT_CODE,md.DISTRICT_NAME,ms.STATE_CODE,ms.REGION_CODE,ms.COUNTRY_CODE  from "MDB_TEST_INTEGRATION"."MST_DISTRICT" as md inner join "MDB_TEST_INTEGRATION"."MST_STATE" as ms on md.STATE_CODE = ms.STATE_CODE inner join "MDB_TEST_INTEGRATION"."MST_COUNTRY" as cm on ms.COUNTRY_CODE = cm.COUNTRY_CODE where md.soft_del = ? ';
			...
			'select md.DISTRICT_CODE,md.DISTRICT_NAME,ms.STATE_CODE,ms.REGION_CODE,ms.COUNTRY_CODE  from "MDB_TEST_INTEGRATION"."MST_DISTRICT" as md inner join "MDB_TEST_INTEGRATION"."MST_STATE" as ms on md.STATE_CODE = ms.STATE_CODE inner join "MDB_TEST_INTEGRATION"."MST_COUNTRY" as cm on ms.COUNTRY_CODE = cm.COUNTRY_CODE where md.STATE_CODE = ? and md.soft_del = ?  ';
...
function addDistrict() {
	...
	try {
		record = {};
		var queryselect = 'select * from "MDB_TEST_INTEGRATION"."MST_DISTRICT" where STATE_CODE=? and DISTRICT_CODE=?';
		...
		if (rsSelect.next()) {
			...
			var CalladdDistrict = 'insert into  "MDB_TEST_INTEGRATION"."MST_DISTRICT"("DISTRICT_CODE","DISTRICT_NAME","STATE_CODE","CREATE_BY") values(?,?,?,?)';
		...
		//	pstmtCalladdDistrict.execute();
			var rs = pstmtCalladdDistrict.executeUpdate();
					connection.commit();
					record = {};
					if (rs > 0) {
						record.status = 1;
...
function deletedistrict(){
	...
	var districtCode = $.request.parameters.get('districtCode');
	try {
			var qryDeleteDistrict = 'update "MDB_TEST_INTEGRATION"."MST_DISTRICT" set SOFT_DEL=? where DISTRICT_CODE=?';
...
var record = {};
	...
	try {
			var qryDeleteDistrict = 'update  "MDB_TEST_INTEGRATION"."MST_DISTRICT" set district_Name=?  , SOFT_DEL =?,MODIFIED_BY=?,MODIFIED_DATE=? where DISTRICT_CODE=? and STATE_CODE=?';
			var pstmtDeleteDistrict = connection.prepareStatement(qryDeleteDistrict);
