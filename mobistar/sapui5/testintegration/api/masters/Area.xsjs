...
function getDistrictAreas(districtCode, output) {
	...
	var qryArea = 'SELECT * from "MDB_TEST_INTEGRATION"."MST_AREA" WHERE DISTRICT_CODE =?';
...
function getAreaDetails(areaCode, output) {
	...
	try {
		var connection = $.db.getConnection();
		var CallArea = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.dev.procedure::Area_CRUD"(?,?,?,?);';
		...
		var CallProZONE = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.dev.procedure::Area_CRUD"(?,?,?,?)';
		...
		while (rsCallProZONE.next()) {
			...
			var CallProBRANCH = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.dev.procedure::Area_CRUD"(?,?,?,?)';
...
function getAreas() {
	...
	try {
        ...
        if(areaStatus === "Deleted") {
            qryAreas = 'SELECT * from "MDB_TEST_INTEGRATION"."MST_AREA" where SOFT_DEL =\'1\'';
        } else if(areaStatus === "Active") {
            qryAreas = 'SELECT * from "MDB_TEST_INTEGRATION"."MST_AREA" where SOFT_DEL=\'0\'';
        } else {
            qryAreas = ' SELECT ma.AREA_CODE,ma.AREA_DESC,ma.DISTRICT_CODE,ma.SOFT_DEL,ma.CREATE_BY,ma.CREATE_DATE, '
+ ' md.DISTRICT_NAME,ms.STATE_NAME,mr.REGION_NAME '
+ ' from "MDB_TEST_INTEGRATION"."MST_AREA" as ma '
...
function addArea() {
	...
	try {
		record = {};
		var queryselect = 'select * from "MDB_TEST_INTEGRATION"."MST_AREA" where area_code=?';
		...
		if (rsSelect.next()) {
			...
			var CallPro = 'insert into  "MDB_TEST_INTEGRATION"."MST_AREA"("AREA_CODE","AREA_DESC","DISTRICT_CODE","CREATE_BY") values(?,?,?,?)';
...
function deleteArea(){
	...
	var areaCode = $.request.parameters.get('areaCode');
	try {
			var qryDeleteArea = 'update "MDB_TEST_INTEGRATION"."MST_AREA" set soft_Del=? where AREA_CODE=?';
...
function updateArea(){
   ...
   var record = {};
	...
	try {
			var qryDeleteArea = 'update  "MDB_TEST_INTEGRATION"."MST_AREA" set 	AREA_DESC=? ,SOFT_DEL=? ,MODIFIED_BY=?,MODIFIED_DATE=? where AREA_CODE=? and DISTRICT_CODE=?';
			var pstmtDeleteArea = con.prepareStatement(qryDeleteArea);
			...
			pstmtDeleteArea.setString(5, areaCode);
			pstmtDeleteArea.setString(6, areaDistrict);
