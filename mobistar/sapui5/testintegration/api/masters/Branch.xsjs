...
var Output = {
	...
	var record = {};
	try {
			var qrydeleteBranch = 'delete  from "MDB_TEST_INTEGRATION"."MST_BRANCH" where BRANCH_CODE=?';
...
var Output = {
	...
	var record = {};
	try {
			var qryupdateBranch = 'update "MDB_TEST_INTEGRATION"."MST_BRANCH" set BRANCH_DESC=?,MODIFIED_BY=?,MODIFIED_DATE=? , SOFT_DEL=? where BRANCH_CODE=? and ZONE_CODE=?';
			var pstmtupdateBranch = connection.prepareStatement(qryupdateBranch);
			pstmtupdateBranch.setString(1, branchDesc);
			pstmtupdateBranch.setString(2, modifiedBy);
			pstmtupdateBranch.setString(3, dateFunction());
			pstmtupdateBranch.setString(4, softDel);
			pstmtupdateBranch.setString(5, branchCode );
			pstmtupdateBranch.setString(6, zoneCode);
...
function getBranch() {
	...
	var BranchCode = $.request.parameters.get('BranchCode');
	try {
		var query = 'Select BRANCH_CODE ,BRANCH_DESC,ZONE_CODE from "MDB_TEST_INTEGRATION"."MST_BRANCH" where BRANCH_CODE = ?';
...
+ ' from "MDB_TEST_INTEGRATION"."MST_BRANCH" as mb '
+ ' inner join "MDB_TEST_INTEGRATION"."MST_ZONE" as mz on mb.ZONE_CODE = mz.ZONE_CODE '
+ ' inner join "MDB_TEST_INTEGRATION"."MST_AREA" as ma on mz.AREA_CODE = ma.AREA_CODE '
+ ' inner join "MDB_TEST_INTEGRATION"."MST_DISTRICT" as md on ma.DISTRICT_CODE = md.DISTRICT_CODE '
...
+ ' inner join "MDB_TEST_INTEGRATION"."MST_REGION" as mr on ms.REGION_CODE = mr.REGION_CODE ';
		...
		while (rsBranch.next()) {
			...
			record.ZoneDesc = rsBranch.getString(7);
			record.AreaDesc = rsBranch.getString(8);
	...
	try {
		if (dataLine.length > 0) {
			for (var i = 0; i < dataLine.length; i++) {
				...
				var queryBranch = 'select *  from "MDB_TEST_INTEGRATION"."MST_BRANCH" where BRANCH_CODE = ? and ZONE_CODE = ?';
				var pstmtBranch = conn1.prepareStatement(queryBranch);
				pstmtBranch.setString(1, dicLine.BRANCH_CODE);
				pstmtBranch.setString(2, dicLine.ZONECODE);
				...
				if (rBranch.next()) {
					...
					var queryInsertBranch =
						'insert into  "MDB_TEST_INTEGRATION"."MST_BRANCH"("BRANCH_CODE","BRANCH_DESC","ZONE_CODE","CREATE_BY") values(?,?,?,?)';
			    	...
			    	pstmtInsertBranch.setString(4, dicLine.createby);
...
// 	var branchArray = [];
	try {
		var connection = $.db.getConnection();
		var CallBranch = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.dev.procedure::Branch_CRUD"(?,?,?,?);';
