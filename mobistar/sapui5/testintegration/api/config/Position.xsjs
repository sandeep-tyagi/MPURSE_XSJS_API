 ...
 */
 ...
 function getPosition() {
 	...
 	try {
 		var CallPro = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.testintegration.procedure::MST_POSITION"(?,?,?,?,?,?,?,?);';
...
function fetchPosition(inputData, connection) {
	var queryselectPosition = 'select * from "MDB_TEST_INTEGRATION"."MST_POSITION" where SOFT_DEL is not null ';
	var whereClause = "";
...
/* function addPosition() {
 	...
 	try {
 		...
 		if (positionName === undefined || positionName === null || positionName === '') {
 			...
 			var qryInsertPosition = 'INSERT INTO  "MDB_TEST_INTEGRATION"."MST_POSITION"(POSITION_NAME) VALUES(?);';
...
/**
 ...
 function updatePosition() {
 	...
 	if (positionIdUpdatePosition === undefined || positionIdUpdatePosition === null || positionIdUpdatePosition === '') {
 		...
 		try {
 			var qryUpdatePosition = 'UPDATE "MDB_TEST_INTEGRATION"."MST_POSITION" SET POSITION_NAME = ?, MODIFIED_DATE=? , SOFT_DEL=? where POSITION_ID= ?';
 ...
 function deletePosition() {
 	...
 	try {
 		if (positionIdDelPosition === undefined || positionIdDelPosition === null || positionIdDelPosition === '') {
 			...
 			var qryDelPosition = 'UPDATE "MDB_TEST_INTEGRATION"."MST_POSITION" SET SOFT_DEL=? where POSITION_ID=?';
