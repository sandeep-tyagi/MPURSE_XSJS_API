...
function getMenu() {
	...
	queryMenuSelect = 'SELECT * FROM "MDB_TEST_INTEGRATION"."MST_MENU" WHERE MENU_ID=? ';
...
function getMenus() {
	...
	try {
		var CallPro = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.testintegration.procedure::MST_MENU"(?,?,?,?,?,?,?,?,?,?);';
...
function fetchMenu(inputData, connection) {
	var queryselectMenu = 'select * from "MDB_TEST_INTEGRATION"."MST_MENU" where SOFT_DEL is not null ';
	var whereClause = "";
...
function addMenu(inputData, connection, record) {
	...
	var queryAddMenu =
		'INSERT INTO "MDB_TEST_INTEGRATION"."MST_MENU"(MENU_NAME,ORDER_NO,URL,HEADER,SUB_HEADER,FRAME_TYPE,HEADER_IMAGE,FOOTER,TILE_TYPE,PRESS,CREATE_BY) values(?,?,?,?,?,?,?,?,?,?,?)';
...
function updateMenu() {
	...
	try {
		queryMenuUpdate =
			'UPDATE "MDB_TEST_INTEGRATION"."MST_MENU" SET MENU_NAME=?,ORDER_NO=?,URL=?,HEADER=?,SUB_HEADER=?,FRAME_TYPE=?,HEADER_IMAGE=?,FOOTER=?,TILE_TYPE=?,PRESS=?,SOFT_DEL=?,MODIFIED_DATE=?,MODIFIED_BY=? where MENU_ID=?';
...
function deleteMenu() {
	...
	try {
		var DelCallPro = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.testintegration.procedure::MST_MENU"(?,?,?,?,?,?,?,?,?,?);';
