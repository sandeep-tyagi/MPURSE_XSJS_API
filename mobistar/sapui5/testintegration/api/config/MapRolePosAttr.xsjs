...
function getMapRolePosAttr() {
	...
	try {
		var CallProRolePosAttr = 'call "MDB_TEST_INTEGRATION"."com.mobistar.sapui5.testintegration.procedure::MAP_ROLE_POS_ATTR"(?,?,?,?,?,?,?,?,?,?)';
...
function deleteMapRolePosAttr(dataLine,status){
    var connection = $.db.getConnection();
		if (dataLine.length > 0) {
			...
			var qryDeleteRoleAtt = 'Delete from "MDB_TEST_INTEGRATION"."MAP_ROLE_POS_ATTR" where ROLE_POS_ID = ? and SUB_MENU_ID in ' +
			                        '(select SUBMENU_ID from "MDB_TEST_INTEGRATION"."MST_SUB_MENU" where MENU_ID = ?)';
...
function checkRolePosAttrRecord(rSubMenu, rAttr, records,rolePositionId) {
	var connection = $.db.getConnection();
	var querycheckRolePosAttr = 'select * from "MDB_TEST_INTEGRATION"."MAP_ROLE_POS_ATTR" where SUB_MENU_ID = ? and ATTRIBUTE_ID = ? ' +
...
function getSubMenuRolePositionAttr() {
	...
	try {
		...
		while (rMenu.next()) {
			var record = {};
			var SubMenu = [];
			var querySubMenu = 'select distinct sub_menu_name,submenu_id,menu_id from "MDB_TEST_INTEGRATION"."MST_SUB_MENU" ' +
